...
- New `tools/` directory for development scripts that support the repo
  ...
  before writing the fixture and its goal file; `patch-goal-files.py`
  re-syncs existing goal files after a change that alters only how the tree
  is rendered.
- New `IrregularTocCheck`: rebuilds a `TOC` whose entries were never nested
  ...
  visual lines, and nesting depth comes from left-edge indentation, so TOCs
  whose entries carry no links are handled the same way. A line that wraps is
  folded back into the entry it continues.
...
- New `BadlyMarkedContentCheck`: flags a paragraph or heading whose single
  ...
  heading run into its subtitle, and splits it into one same-role element per
...
- New `!SPLIT_LINES [spec]` scribble instruction: splits marked-content
  ...
  own MCID. Handles elements holding several MCRs, even spanning pages;
  joins an enclosing list or wraps a bare paragraph in a new one. The optional spec is an expected
...
- Added `MistaggedHeadingCheck` that flags paragraphs and mis-leveled
  ...
  retagging at the appropriate heading level. Headings that would skip a
  level (e.g. H3 to H5, or a subheading before any top-level heading) are
  reported separately as improperly nested for manual review rather than
  retagged.
...
- `--dump-tree` also appends text MCR content inside Link and H1-H6 elements.
